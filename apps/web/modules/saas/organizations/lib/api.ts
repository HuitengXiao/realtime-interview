...
export const organizationJoinCodesQueryKey = (organizationId: string) =>
	["organization", organizationId, "join-codes"] as const;

export const useOrganizationJoinCodesQuery = (organizationId: string) =>
	useQuery({
		queryKey: organizationJoinCodesQueryKey(organizationId),
		queryFn: async () => {
			const response = await apiClient.organizations[":organizationId"][
				"join-codes"
			].$get({ param: { organizationId } });
			if (!response.ok) {
				throw new Error("Failed to fetch team join codes");
			}
			return response.json();
		},
	});

export const useCreateOrganizationJoinCodeMutation = (organizationId: string) =>
	useMutation({
		mutationFn: async (expiresInHours: number) => {
			...
			if (!response.ok) {
				throw new Error("Failed to create team join code");
			}
			return response.json();
		},
	});

export const useUpdateOrganizationJoinCodeMutation = (organizationId: string) =>
	useMutation({
		mutationFn: async ({
			id,
			revoked,
			...
			if (!response.ok) {
				throw new Error("Failed to update team join code");
			}
			return response.json();
		},
	});
...
export const useJoinOrganizationByCodeMutation = () =>
	useMutation({
		mutationFn: async (code: string) => {
			...
			if (!response.ok) {
				throw new Error("Invalid or expired team join code");
			}
			return response.json();
		},
	});
